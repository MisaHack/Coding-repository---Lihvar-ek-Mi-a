...
function addLink(){
   ...
   request.onsuccess = function(event){
      ...
      //Refreh the display.
      //(For better performance, you could add just the one new item,
...
function showLinks(){
   ...
   //Prepare the string of markup that will be
   //inserted into the page
   var markupToInsert = "";

   request.onscuccess = function(event){
      //Create a cursor.
      ...
      if(cursor){
         ...
         markupToInsert +=
         "<a href=" + linkRecord.url + ">" + linkRecord.name + "</a> (" +
         ...
         //When the cursor steps to the next record
         //(or reaches the end, the request.onsuccess event fires again.)
         cursor.continue();
      }
      else{
         //You've reached the end of the results.

         //If there wasn't at least one result,
         //substitute some placeholder text.
         if(markupToInsert == ""){
            markupToInsert = "<< No links. >>";
         }
         else{
            markupToInsert =
            "<i>Links you've added so far: </i><br>" +
            markupToInsert;
         }
...
function getLinkDetails(element){
   ...
   request.onsuccess = function(event){
      ...
      var linkRecord = request.results;

      var resultsElement = document.getElementById("linkDetails");
      resultElement.innerHTML = "<b>" + linkRecord.name + "</b><br>" +
