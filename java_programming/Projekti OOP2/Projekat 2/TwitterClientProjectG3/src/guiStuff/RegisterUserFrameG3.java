...
public class RegisterUserFrameG3 extends JFrame {
	/**
	 * @author Nemanja Dragin
	 * GUI za registrovanje
	 */
	 ...
	 */
	public RegisterUserFrameG3() throws NamingException {
		...
		JPanel panTweetName = new JPanel();
		getContentPane().add(panTweetName);
		setVisible(true);
		panTweetName.setLayout(new BoxLayout(panTweetName, BoxLayout.X_AXIS));
		...
		panTweetName.add(horizontalStrut);
		JLabel label = new JLabel("Twitter name : ");
		panTweetName.add(label);
		//
		Component horizontalGlue_8 = Box.createHorizontalGlue();
		panTweetName.add(horizontalGlue_8);
		
		txtTweetName = new JTextField();
		txtTweetName.setPreferredSize(new Dimension(250, 20));
		txtTweetName.setMinimumSize(new Dimension(250, 20));
		txtTweetName.setMaximumSize(new Dimension(250, 20));
		panTweetName.add(txtTweetName);
		
		Component horizontalStrut_12 = Box.createHorizontalStrut(20);
		panTweetName.add(horizontalStrut_12);
		...
		btnRegister.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				String nameUser;
				if((nameUser = txtName.getText()).length() < 3 || (nameUser = txtName.getText()).length() >20){
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Your name must have beetween 3 and 20 chars!", "Error!", JOptionPane.ERROR_MESSAGE);
				...
				String surnameUser;
				if((surnameUser = txtSurname.getText()).length() < 3 || (surnameUser = txtSurname.getText()).length() > 20){
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Your surname must have beetween 3 and 20 char!s", "Error!", JOptionPane.ERROR_MESSAGE);
				...
				String emailUser;
				if((emailUser = txtEmail.getText()).length() < 5 || (emailUser = txtEmail.getText()).length() > 40){
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Your email must have beetween 5 and 40 chars!", "Error!", JOptionPane.ERROR_MESSAGE);
					txtEmail.requestFocus();
					return;
				}
				emailUser = txtEmail.getText().trim();	
				
				String passUser;
				if((passUser = txtPassword.getText()).length() < 5 || (passUser = txtPassword.getText()).length() > 20){
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Your password must have beetween 5 and 20 chars!", "Error!", JOptionPane.ERROR_MESSAGE);
				...
				String tweetUser;
				if((tweetUser = txtTweetName.getText()).length() < 5 || (tweetUser = txtTweetName.getText()).length() > 20){
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Your tweetname must have beetween 5 and 30 chars!", "Error!", JOptionPane.ERROR_MESSAGE);
					txtTweetName.requestFocus();
					return;
				//ako twitter nadimak pocinje sa @ nastavi dalje, u suprotnom izbaci poruku
				}
				/**
				 * @author Nenad Barugdzic
				 */
				UserG3 user = userRemote.searchUsersByTweetnameFull("@"+txtTweetName.getText());
				if (user == null) {
					tweetUser = "@" + txtTweetName.getText().trim();
				} else {
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Tweetname allready exists! Chose another tweetname.", "Error!", JOptionPane.ERROR_MESSAGE);
					txtTweetName.requestFocus();
					return;
				}
				
				String aboutUser;
				if( (aboutUser = txtTweetName.getText()).length() > 140){
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Your about cant have more than 140 chars!", "Error!", JOptionPane.ERROR_MESSAGE);
					txtTweetName.requestFocus();
					return;
				//ako twitter nadimak pocinje sa @ nastavi dalje, u suprotnom izbaci poruku
				}
				aboutUser = txtAbout.getText().trim();		
				/**
				 * @author Nenad Barugdzic
				 */
				...
				try {
					...
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Wrong date format! Date format must be in 'dd.MM.yyyy' format.", "Error!", JOptionPane.ERROR_MESSAGE);
				 ...
				 * @author Nenad Barugdzic
				 */
				String avatar = "/guiStuff/icons/twitterAvatar.png";
				boolean insert = userRemote.registerUser(emailUser, nameUser, surnameUser, tweetUser, passUser,
						avatar, aboutUser, birthUser);
				if (insert) {
					
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Registration succsessful.");					
					
					LoginUserFrameG3.openLoginPage();
					setVisible(false);
				} else {
					JOptionPane.showMessageDialog(RegisterUserFrameG3.this, "Registration failed.");
				}
			}
		});
	 ...
	 * @author Nenad Barugdzic
	 */
	public void openRegistratonPage(){
		try {
			setVisible(true);
		} catch (Exception e) {
			e.printStackTrace();
